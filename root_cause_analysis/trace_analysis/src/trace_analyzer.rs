...
pub struct TraceAnalyzer {
    ...
    pub address_scores: HashMap<(usize, usize), Predicate>,
...
fn store_trace(trace: &Trace, must_have: &Option<HashSet<usize>>) -> bool {
    match must_have {
        Some(addresses) => {
            for (key, _) in &trace.instructions {
            ...
            false
            // addresses.iter().any(|k| trace.instructions.contains_key(k))
        },
        None => true,
    }
}
...
impl TraceAnalyzer {
    ...
    pub fn address_union(&self) -> HashSet<(usize, usize)> {
    ...
    pub fn crash_address_union(&self) -> HashSet<(usize, usize)> {
        TraceAnalyzer::trace_union(&self.crashes)
    }

    fn trace_union(traces: &TraceVec) -> HashSet<(usize, usize)> {
    ...
    pub fn crash_non_crash_intersection(&self) -> HashSet<(usize, usize)> {
        let crash_union = TraceAnalyzer::trace_union(&self.crashes);
        let non_crash_union = TraceAnalyzer::trace_union(&self.non_crashes);
        crash_union
    ...
    pub fn get_any_mnemonic(&self, address: (usize, usize)) -> String {
