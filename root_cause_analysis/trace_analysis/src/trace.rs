...
#[derive(Clone, Serialize, Deserialize, Ord, PartialOrd, PartialEq, Eq)]
pub enum EdgeType {
    Direct,
    Indirect,
    Conditional,
...
impl SerializedTrace {
    pub fn to_trace(name: String, serialized: SerializedTrace) -> Trace {
        let mut instructions: HashMap<(usize, usize), Instruction> = serialized
            .instructions
            .into_iter()
            .map(|instr| ((instr.address, instr.hash), instr.to_instruction()))
            .collect();
        for edge in &serialized.edges {
            let mut stack_hash: usize = 0;
            for (key, _) in &mut instructions {
                if key.0 == edge.from {
                    stack_hash = key.1;
                }
            }
            if let Some(entry) = instructions.get_mut(&(edge.from, stack_hash)) {
...
pub struct Trace {
    pub name: String,
    pub image_base: usize,
    pub instructions: HashMap<(usize, usize), Instruction>,
    pub first_address: usize,
    pub last_address: usize,
}
...
impl Trace {
    ...
    pub fn from_trace_file_serialized(file_path: String) -> SerializedTrace {
        let content =
            fs::read_to_string(&file_path).expect(&format!("File {} not found!", &file_path));
        Trace::from_file_serialized(file_path, content)
    }
    ...
    pub fn from_zip_file_serialized(file_path: String) -> SerializedTrace {
        let zip_file =
            fs::File::open(&file_path).expect(&format!("Could not open file {}", &file_path));
        let mut zip_archive = zip::ZipArchive::new(zip_file)
            .expect(&format!("Could not open archive {}", &file_path));
        ...
        Trace::from_file_serialized(trace_file_path, trace_content)
    }
    ...
    fn from_file_serialized(file_path: String, content: String) -> SerializedTrace {
        serde_json::from_str(&content)
            .expect(&format!("Could not deserialize file {}", &file_path))
    }

    pub fn visited_addresses(&self) -> HashSet<(usize,usize)> {
