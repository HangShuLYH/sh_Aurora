...
impl PredicateBuilder {
    fn gen_visited(address: (usize, usize)) -> Vec<Predicate> {
        vec![Predicate::new(
            "is_visited",
            address,
            is_visited,
    ...
    fn gen_all_edge_from_to_predicates(
        ...
        cfg.get_instruction_successors(address.0)
            .iter()
            .map(|to| {
                let pred_name = format!("0x{:x} {} {} 0x{:x}", address.0, address.1 , pred_name, to);
    ...
    pub fn gen_flag_predicates(address: (usize, usize), trace_analyzer: &TraceAnalyzer) -> Vec<Predicate> {
    ...
    pub fn gen_cfg_predicates(address: (usize, usize), cfg: &ControlFlowGraph) -> Vec<Predicate> {
        let mut ret = vec![];

        // check if end of basic block
        if !cfg.is_bb_end(address.0) {
            return ret;
        }
    ...
    pub fn gen_predicates(address: (usize, usize), trace_analyzer: &TraceAnalyzer) -> Vec<Predicate> {
        let mut ret = vec![];
