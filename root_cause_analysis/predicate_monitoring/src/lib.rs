...
use nix::unistd::Pid;
use predicate::*;
use ptracer::{ContinueMode, Ptracer, read};
use core::ffi::c_void;
// use ptracer::*;
use register::*;
use rflags::RFlags;
...
) -> HashMap<(usize, usize), RootCauseCandidate> {
    predicates
        .into_iter()
        .map(|pred| {
            debug!("pred = {:?}", pred);
            ...
            let instr =
                instruction(&decoder, dbg.pid, address.0).expect("failed to parse instruction");
...
fn insert_breakpoints(dbg: &mut Ptracer, rccs: &HashMap<(usize, usize), RootCauseCandidate>) {
    for address in rccs.keys() {
        dbg.insert_breakpoint(address.0)
...
fn add_rccs_single_steps(
    pid: Pid,
    dbg: &mut Ptracer,
    rccs: &HashMap<(usize, usize), RootCauseCandidate>,
    single_steping: &mut HashMap<Pid, (nix::libc::user_regs_struct, usize)>,
) {
    let rip = dbg.registers.rip;
    let mut rbp = dbg.registers.rbp as usize;
    let mut stack_hash: usize = 0;
    // println!("-------------");
    while rbp != 0 {
        let ret_addr:usize = read(pid, (rbp + 8) as *mut usize as *mut c_void).expect("Failed to read ret_addr") as usize;
        rbp = read(pid, rbp as *mut usize as *mut c_void).expect("Failed to read rbp") as usize;
        if rbp == 0 {
            break;
        }
        stack_hash = (stack_hash >> 1) ^ ret_addr;
        // println!("ret_addr:{}", ret_addr);
    }
    // println!("addr:{}; stack_hash:{}", rip, stack_hash);
    if let Some(rcc) = rccs.get(&(dbg.registers.rip as usize, stack_hash)) {
        debug!(
            "breakpoint at {:#018x} of predicate {:016x?} reached",
            rip, rcc.predicate
        );

        single_steping.insert(pid, (dbg.registers, stack_hash));
    }
}
...
fn check_rccs(
    ...
    old_registers: &nix::libc::user_regs_struct,
    old_hash: usize,
    rccs: &mut HashMap<(usize, usize), RootCauseCandidate>,
    satisfaction: &mut Vec<((usize,usize), Predicate)>,
) {
    let old_rip = old_registers.rip;
    ...
    if let Some(rcc) = rccs.get(&(old_rip as usize, old_hash)) {
        debug!(
        ...
        println!(
            "single step target at {:#018x} {} of predicate {:016x?} reached",
        dbg.registers.rip, old_hash, rcc.predicate
        );
        if !rcc
            ...
            .expect("failed to test predicate")
        {
            ...
            println!("predicate {:016x?} NOT satisfied", rcc.predicate);
            return;
        }
    } else {
    ...
    // predicate satisfied
    if let Some(rcc) = rccs.remove(&(old_rip as usize, old_hash)) {
        info!(
...
fn collect_satisfied(
    ...
    loop {
        ...
        if let Some(pid) = dbg.event().pid() {
            ...
            if let Some((old_registers, old_hash)) = single_steping.remove(&pid) {
                // handle previous single steping request
                check_rccs(dbg, &old_registers, old_hash, rccs, &mut satisfaction)
            } else {
    ...
    println!("satisfaction = {:#018x?}", satisfaction);
    satisfaction
}
