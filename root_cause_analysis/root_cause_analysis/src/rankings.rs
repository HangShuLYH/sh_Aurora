...
fn path_rank(address: (usize, usize), rankings: &Vec<Vec<(usize, usize)>>) -> f64 {
    rankings
        .par_iter()
...
fn rank_path_level(address: (usize, usize), rank: &Vec<(usize, usize)>) -> f64 {
...
pub fn serialize_rankings(config: &Config, rankings: &Vec<Vec<(usize, usize)>>) {
...
fn deserialize_rankings(config: &Config) -> Vec<Vec<(usize, usize)>> {
    ...
    let content: String = predicates
        ...
        .map(|p| {
            format!(
                "{} -- {} (path rank: {})\n",
                p.to_string(),
                mnemonics[&p.address.0],
                path_rank(p.address, rankings)
            )
        })
        .collect();
    write_file(
        &format!("{}/sh_ranked_predicates.txt", config.eval_dir),
        content,
    );
}
